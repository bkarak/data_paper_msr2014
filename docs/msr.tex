...
When the processing was completed, we ran a series of sanity checks on the results, to check their validity. We discovered several issues, which we addressed with various approaches. The most common problem was Findbugs out-of-memory crashes, which demanded to repeat the process for those {\sc jar}s with the appropriate settings in the Java runtime environment. The sanitisation process was consisted of four iterations, until all our scripts indicate that our data was correct. 
...
\hline
\end{tabular}
\caption{Size metrics for the tools}
