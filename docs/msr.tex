...
numerous types of bugs. In this paper we present the dataset
that was produced from this experiment.
The dataset contains the metric results that FindBugs reports for
every project version included in the ecosystem. For every
version we also stored specific metadata such as the {\sc jar}'s
size, it's dependencies and others. Finally, we present
how researchers can use our dataset to produce interesting results.
\end{abstract}
